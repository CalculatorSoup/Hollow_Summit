...
namespace Summit
{
    public class InstantiateArtifactPortal : NetworkBehaviour
    {
        ...
        void OnValidate()
        {
            ...
            if(artifactButtons.Length != 9)
            {
                UnityEngine.Debug.LogError("Number of buttons is not equal to 9. This WILL result in artifact portal not spawning in game!");
            }
#endif
        }
        ...
        public static void CreateAndRegisterLaptop(ContentPack contentPack)
        {
            ...
            if (!laptopTransform)
            {
                Log.Warning($"Couldn't find PortalDialer in {entireIsland}. Not registering laptop to NetworkedObjects catalog...");
                return;
            }

            laptopPrefab = laptopTransform.gameObject.InstantiateClone("LOP_ArtifactLaptop", true);

            UnityEngine.Object.DestroyImmediate(laptopPrefab.transform.Find("spmSMGrassSmallCluster (3)").gameObject);
            UnityEngine.Object.DestroyImmediate(laptopPrefab.transform.Find("spmSMGrassSmallCluster (4)").gameObject);
            UnityEngine.Object.DestroyImmediate(laptopPrefab.transform.Find("SM_PowerLine").gameObject);
