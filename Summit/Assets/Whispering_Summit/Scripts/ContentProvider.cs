...
namespace Summit.Content
{
    ...
    {
        public string identifier => Summit.GUID + "." + nameof(ContentProvider);
        ...
        public IEnumerator LoadStaticContentAsync(LoadStaticContentAsyncArgs args)
        {
            ...
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, SummitContent.ScenesAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => scenesAssetBundle = assetBundle);

            AssetBundle assetsAssetBundle = null;
            yield return LoadAssetBundle(
                Path.Combine(assetsFolderFullPath, SummitContent.AssetsAssetBundleFileName),
                args.progressReceiver,
                (assetBundle) => assetsAssetBundle = assetBundle);

            yield return SummitContent.LoadAssetBundlesAsync(
